...
try {
  ...
  // List the top 5 largest losses
  losingTrades.sort((a,b) => (a.pnl_usd || 0) - (b.pnl_usd || 0));
  console.log("=== LARGEST LOSSES ===");
  losingTrades.slice(0, 10).forEach(t => {
      console.log(`Date: ${new Date(t.timestamp).toISOString()} | PnL: $${t.pnl_usd?.toFixed(2)} | Reason: ${t.close_reason} | Side: ${t.signal_side} | Size: ${t.size}`);
  });
} catch (e) {
  console.error(e);
}
