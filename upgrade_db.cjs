...
// Add it to the JSON decoding layer so the engine gets it
...
code = code.replace(new RegExp(decodeTarget, 'g'), decodeNew);

fs.writeFileSync('tradeDatabaseManager.ts', code, 'utf8');
console.log("Upgraded tradeDatabaseManager.ts for 10m counterfactuals");
